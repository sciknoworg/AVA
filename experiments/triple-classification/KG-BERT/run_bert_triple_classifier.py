...
class KGProcessor(DataProcessor):
    ...
    def corrupt_all_head_tail(ent2text, entities, examples, i, line, set_type, text_a, text_b, text_c, corrupt_items=20):
      """
    Generate multiple negative triples by corrupting both head and tail.

    Test-Time Ranking Setup:
    For each positive triple, create:
    - `corrupt_items` head-corrupted negatives
    - `corrupt_items` tail-corrupted negatives

    With the default value (`corrupt_items=20`), this gives:
    - 1 positive triple
    - 20 head corruptions
    - 20 tail corruptions

    Total candidate group size = 41, which is later used for
    ranking-based evaluation (MRR / Hits@K).
    """
    ...
    def _create_examples(self, lines, set_type, data_dir):
        """
        Create `InputExample` objects for train, dev, and test splits.

        Processing Flow
        ---------------
        1) Load entity and relation text mappings.
        2) Convert triple ids into natural-language text.
        3) Add the positive triple first.
        4) Generate negative examples depending on split:
        - train: one corrupted negative
        - dev: use provided labels if available, otherwise generate one negative
        - test: use provided labels if available, otherwise generate a full
            ranking candidate set (1 positive + 40 negatives)

        Parameters
        ----------
        lines:
            Raw TSV rows containing triples.
        set_type:
            Split name: "train", "dev", or "test".
        data_dir:
            Directory containing `entity2text.txt` and `relation2text.txt`.

        Returns
        -------
        List[InputExample]
            Prepared examples for the selected dataset split.
    """
        ent2text = {}
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == "train":
                ...
                self.corrupt_head_tail(
                    ent2text, entities, examples, i, line, lines_str_set, set_type, text_a, text_b, text_c
                )
                ...
                if len(line) >= 4 and line[3] in ["0", "1"]:
                    ...
                    # positive triple first
                    examples.append(
                        InputExample(guid=guid, text_a=text_a, text_b=text_b, text_c=text_c, label="1")
                    )
                    # ranking candidate set
                    self.corrupt_all_head_tail(
                        ent2text, entities, examples, i, line, set_type, text_a, text_b, text_c, corrupt_items=20
                    )
...
def tc_compute_metrics(logits, group_size=41):
    ...
    for triple_id in range(0, len(logits), group_size):
        ...
        # true triple is always first in group
...
def main():
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        result = tc_compute_metrics(preds, group_size=41)
        loss = tr_loss / nb_tr_steps if args.do_train else None
